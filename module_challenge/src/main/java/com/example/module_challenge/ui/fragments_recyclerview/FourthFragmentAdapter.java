...
public class FourthFragmentAdapter extends RecyclerView.Adapter<FourthFragmentViewHolder>{
    ...
    public FourthFragmentAdapter(List<ChallengePunch> list , TextView progress){
        this.punchList = list;
    ...
    @NonNull
    @Override
    public FourthFragmentViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        database = challenge_Database.getInstance(parent.getContext());
        dao = database.getChallenge_data_dao();
        View view = View.inflate(parent.getContext(), R.layout.layout_fragment_fourth_carrying_recyclerview , null);
        FourthFragmentViewHolder fourthFragmentViewHolder = new FourthFragmentViewHolder(view);
        return fourthFragmentViewHolder;
    }

    @Override
    public void onBindViewHolder(@NonNull FourthFragmentViewHolder holder, int position) {
        int layer = position;
        ...
        holder.card.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(layer == progress){
                    //完成当前挑战
                    dao.updateChallenge_data(new challenge_data(4,challenge,progress+1,complete));
                ...
                if(progress == 6){
                    ...
                    dao.updateChallenge_data(new challenge_data(4 ,0,0,1));
                }
            }
        });
    }
...
class FourthFragmentViewHolder extends RecyclerView.ViewHolder{
    ...
    ImageView imageView;
    View card;
    public FourthFragmentViewHolder(@NonNull View itemView) {
        super(itemView);
        day = itemView.findViewById(R.id.challenge_fourthFragment_carrying_textView);
        taskName = itemView.findViewById(R.id.challenge_fourthFragment_carrying_taskName);
        taskDescription = itemView.findViewById(R.id.challenge_fourthFragment_carrying_taskDescription);
        imageView = itemView.findViewById(R.id.challenge_fourthFragment_carrying_image);
        card = itemView.findViewById(R.id.challenge_fourthFragment_carrying_constraint);
    }
}